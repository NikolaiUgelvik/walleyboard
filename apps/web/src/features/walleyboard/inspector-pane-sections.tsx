...
export function DraftEditorFields({
  controller,
}: {
...
function DraftActionRow({
  ...
  controller: InspectorPaneController;
  selectedDraft: InspectorPaneController["selectedDraft"];
  variant: "new" | "draft";
}) {
  const isNewDraft = variant === "new";
      ...
      {isNewDraft ? (
        ...
        <Button
          ...
          loading={
            ...
            controller.deleteDraftMutation.variables === selectedDraft?.id
          }
          onClick={() => {
            if (!selectedDraft) {
              return;
        ...
        {isNewDraft ? (
          ...
          <>
            <Button
              ...
              variant="light"
              disabled={
                !controller.draftFormDirty || controller.draftAnalysisActive
              }
              loading={
            ...
            <Button
              ...
              loading={
                ...
                controller.refineDraftMutation.variables === selectedDraft?.id
              }
              onClick={() => {
                if (!selectedDraft) {
                  return;
            ...
            <Button
              ...
              onClick={() => {
                ...
                controller.revertDraftRefineMutation.mutate(selectedDraft.id);
              }}
            >
              Revert Refine
            </Button>
            <Button
              ...
              loading={
                ...
                controller.questionDraftMutation.variables === selectedDraft?.id
              }
              onClick={() => {
                if (!selectedDraft) {
                  return;
...
export function NewDraftInspectorSection({
  controller,
}: {
  controller: InspectorPaneController;
}) {
  if (!controller.draftEditorProject) {
    return null;
  }
...
export function DraftInspectorSection({
  controller,
}: {
  controller: InspectorPaneController;
}) {
      ...
      <Stack gap="md">
        ...
        <Box className="detail-meta-grid">
          ...
          <Box className="detail-meta-card">
            ...
            <Text fw={700}>
              {controller.selectedDraft.proposed_acceptance_criteria.length}
            </Text>
          </Box>
        </Box>
        ...
        {controller.draftFormDirty ? (
          <Text size="sm" c="dimmed">
            Save changes before refining, asking questions, or creating a ready
            ticket.
          </Text>
        ) : null}
...
function SessionWorkflowSection({
  controller,
  selectedSessionTicket,
}: {
  controller: InspectorPaneController;
  selectedSessionTicket: InspectorPaneController["selectedSessionTicket"];
}) {
  if (!controller.session || !selectedSessionTicket) {
    return (
      <Text size="sm" c="dimmed">
        Use the ticket card actions for diff, terminal, preview, and the full
        interpreted activity stream.
      </Text>
    );
  }

  const ticket = selectedSessionTicket;
      ...
      {selectedSessionTicket.status === "review" ? (
        controller.reviewPackageQuery.isPending ? (
          ...
          <Stack gap="sm">
            ...
            <Text size="sm" c="dimmed">
              Diff artifact: <Code>{controller.reviewPackage.diff_ref}</Code>
            </Text>
            <MarkdownContent
              className="markdown-muted markdown-small"
            ...
            {controller.reviewPackage.validation_results.length > 0 ? (
              <List size="sm" spacing={4}>
                {controller.reviewPackage.validation_results.map((result) => (
            ...
            <AgentReviewPanel
              ...
              latestReviewRunPending={controller.latestReviewRunQuery.isPending}
              onOpenHistory={controller.openAgentReviewHistoryModal}
              onStart={() => {
                controller.startAgentReviewMutation.mutate(ticket.id);
              }}
              startError={
                controller.startAgentReviewMutation.isError
                  ? controller.startAgentReviewMutation.error.message
                  : null
              }
              startPending={
            ...
            <Textarea
              ...
              onChange={(event) =>
                controller.setRequestedChangesBody(event.currentTarget.value)
              }
              minRows={3}
            />
            <Group justify="space-between">
              <Button
                ...
                loading={
                  ...
                  controller.requestChangesMutation.variables?.ticketId ===
                    ticket.id
                }
                disabled={controller.requestedChangesBody.trim().length === 0}
                onClick={() => {
              ...
              {(() => {
                ...
                return [reviewActions.primary, reviewActions.secondary]
                      ...
                      return (
                        <Button
                          ...
                          className={projectAccentButtonClassName(
                            reviewActions.primary?.kind === "create_pr"
                              ? "filled"
                              : "default",
                          )}
                          variant={
        ...
        <form
          onSubmit={(event) => {
            ...
            controller.planFeedbackMutation.mutate({
              sessionId: controller.selectedSessionId as string,
              approved: true,
              body:
        ...
        >
          <Stack gap="sm">
            <Textarea
              ...
              onChange={(event) =>
                controller.setPlanFeedbackBody(event.currentTarget.value)
              }
              minRows={3}
            />
            <Group justify="space-between">
              <Button
                ...
                variant="light"
                type="button"
                disabled={controller.planFeedbackBody.trim().length === 0}
                loading={
                  controller.planFeedbackMutation.isPending &&
                  controller.planFeedbackMutation.variables?.approved === false
                }
                onClick={() => {
                  controller.planFeedbackMutation.mutate({
                    sessionId: controller.selectedSessionId as string,
              ...
              <Button
                ...
                loading={
                  controller.planFeedbackMutation.isPending &&
                  controller.planFeedbackMutation.variables?.approved === true
                }
              >
        ...
        >
          <Stack gap="sm">
            <Textarea
              ...
              onChange={(event) =>
                controller.setResumeReason(event.currentTarget.value)
              }
              minRows={3}
            />
            ...
            <Group justify="space-between">
              ...
              <Group gap="sm">
                ...
                <Button
                  className={projectAccentButtonClassName("filled")}
                  type="submit"
                  loading={controller.resumeTicketMutation.isPending}
                >
                  Resume Execution
                </Button>
              </Group>
...
export function SessionInspectorSection({
  activitySummary,
  controller,
}: {
  activitySummary: string | null;
  ...
  if (controller.sessionQuery.isError) {
    return (
      <SectionCard
        title="Ticket session"
      ...
      <SectionCard>
        <Stack gap="md">
              ...
              <Box className="detail-meta-card">
                ...
                <Text fw={700}>
                  {controller.repositories.find(
                    (repository) =>
                      repository.id === selectedSessionTicket.repo,
                  )?.name ?? "Pending"}
                </Text>
              </Box>
              <Box className="detail-meta-card">
                ...
                <Text fw={700}>
                  {selectedSessionTicket.ticket_type.charAt(0).toUpperCase() +
                    selectedSessionTicket.ticket_type.slice(1)}
                </Text>
              </Box>
              <Box className="detail-meta-card">
                <Text size="xs" c="dimmed" tt="uppercase" fw={700}>
                ...
                <Badge
                  variant="light"
                  color={ticketStatusColor(selectedSessionTicket.status)}
                  style={{ alignSelf: "flex-start" }}
                >
                  {humanizeTicketStatus(selectedSessionTicket.status)}
                </Badge>
              </Box>
                ...
                <Box className="detail-meta-card">
                  ...
                  <Group gap="xs" wrap="wrap">
                    <PullRequestStatusBadge
                      linkedPr={selectedSessionTicket.linked_pr}
                    />
                    <Text
                      component="a"
                      href={selectedSessionTicket.linked_pr.url}
                      target="_blank"
                      rel="noreferrer"
                      size="sm"
                    >
          ...
          {selectedSessionTicket ? (
            <Stack gap="xs">
              ...
              <MarkdownContent
                ...
                onTicketReferenceNavigate={controller.navigateToTicketReference}
                ticketReferences={selectedSessionTicket.ticket_references ?? []}
              />
              {selectedSessionTicket.acceptance_criteria.length > 0 ? (
                <Stack gap={2}>
                  <Text size="xs" c="dimmed" tt="uppercase" fw={700}>
                    Acceptance Criteria
                  </Text>
                  <MarkdownListItems
                    items={selectedSessionTicket.acceptance_criteria}
                  />
                </Stack>
              ) : null}
            </Stack>
          ...
          {controller.session.status === "queued" ? (
            <Text size="sm" c="dimmed">
              This ticket is in progress and waiting for one of the project's
              running slots to open.
            </Text>
          ) : null}
          ...
          {selectedSessionTicket ? (
            <Group justify="space-between">
                ...
                isStoppableSessionStatus(
                  ...
                  <Button
                    ...
                    loading={
                      ...
                      controller.stopTicketMutation.variables?.ticketId ===
                        selectedSessionTicket.id
                    }
                    onClick={() => {
              ...
              <Button
                ...
                onClick={() => controller.deleteTicket(selectedSessionTicket)}
              >
                Delete Ticket
              </Button>
            </Group>
...
export function TicketInspectorSection({
  controller,
}: {
  controller: InspectorPaneController;
}) {
  if (!controller.selectedInspectorTicket) {
    return null;
  }
