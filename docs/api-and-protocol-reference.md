...
- Backend alert events are emitted as `protocol.event` envelopes with `event_type: "inbox.alert"`.
- Alerts are backend-authored and are not replayed on reconnect.
...
`inbox.alert` is the backend-owned in-app notification event. It is emitted only when the backend observes a settled, human-actionable state.
...
```text
{
  ...
  payload: {
    ...
    alerts: [
      {
        kind: "draft" | "review" | "session",
        notification_key: string,
        project_id: string,
        item_key: string,
        target_kind: "draft" | "session",
        target_id: string,
        draft_id?: string,
        ticket_id?: number,
        session_id?: string,
...
The frontend uses this event to play the notification sound and keep the inbox UI in sync without deriving alerts from client-side snapshot diffs.
...
| `inbox.alert` | `ProtocolEvent` on `/events` | plays the alert sound for settled human-actionable inbox items |
